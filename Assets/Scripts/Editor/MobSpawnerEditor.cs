...
namespace LushWorld.Editor
{
    ...
    public class MobSpawnerEditor : UnityEditor.Editor
    {
        ...
        {
            var spawner = (MobSpawner)target;
            Handles.color = new Color(0f, 1f, 0.5f, 0.12f);
        ...
        private void Scatter()
        {
            ...
            string msg = $"[MobSpawner] Scattered {created.Count}/{_count} spawn points within {_radius}m of '{spawner.name}'.";
        ...
        {
            var toDestroy = new List<GameObject>();
            foreach (Transform child in ((MobSpawner)target).transform)
                if (child.name.StartsWith("SpawnPoint_"))
                    toDestroy.Add(child.gameObject);
            foreach (var go in toDestroy)
                Undo.DestroyObjectImmediate(go);
