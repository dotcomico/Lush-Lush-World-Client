...
- `MobilePlacementControls` — **attach to `MobilePlacementPanel`** (parent of PlaceButton + RotateButton in PlayerRig.prefab); already attached via prefab YAML; on non-mobile disables itself entirely in `Awake`; uses `CanvasGroup` (auto-added) to show/hide panel on `BuildingSystem.OnPlacementStateChanged`; PlaceButton: wires `UIVirtualButton.buttonClickOutputEvent` (tap) → `BuildingSystem.MobilePlacePressed()`; RotateButton: adds `PointerHoldRelay` component at runtime (no Inspector wiring needed) — fires `true` on finger-down, `false` on finger-up → `BuildingSystem.MobileRotateHeld(bool)`; buttons found by name via `transform.Find`
- `PointerHoldRelay` (internal class inside `MobilePlacementControls.cs`) — `IPointerDownHandler` + `IPointerUpHandler`; added via `AddComponent` at runtime to RotateButton; exposes `Action<bool> OnHeldChanged`; never appears in the Inspector
- `MobileSkeletonControls` — attach to `MobileSkeleton` panel (parent of `Delete` + `AddMterials` buttons) inside PlayerRig; self-disables on desktop in `Awake`; subscribes to `ResourceInteractor.OnNearBlueprintChanged`; shows/hides via `CanvasGroup` (auto-added); `Delete` button → `OnDeletePressed()` → `bp.Demolish()`; `AddMterials` button → `OnAddMaterialsPressed()` → `bp.Interact()`; wire buttons via Inspector `OnClick`
- Prefabs: `Assets/App/Prefabs/PlayerRig.prefab` — `UI_Canvas_StarterAssetsInputs_Joysticks > MobilePlacementPanel` (direct children: `RotateButton`, `PlaceButton`); `MobilePlacementControls` component is on `MobilePlacementPanel`; `MobileSkeleton` panel holds `Delete` + `AddMterials` buttons with `MobileSkeletonControls` component
...
### Tongue Attack
- Script: `Assets/Scripts/Player/TongueAttack.cs` — on `PlayerCapsule`; subscribes to `PlayerInput.actions["Player/Attack"].performed`; coroutine-driven extend/detect/retract cycle; `RequestAttack()` is the NGO-ready entry point (Phase 2: `[ServerRpc]`); exposes `static LocalPlayer` (set in Awake, cleared in OnDestroy) for mobile button access
- Mobile button: `Assets/Scripts/UI/MobileAttackButton.cs` — attach to `Attack` button GO in `UI_Canvas_StarterAssetsInputs_Joysticks`; `OnAttackButtonPressed()` → `TongueAttack.LocalPlayer?.RequestAttack()`; pair with `MobileCanvasVisibility` on the same GO to hide on desktop
...
- Scripts done:
  ...
  - `Assets/Scripts/UI/Building/BuildingMenuUI.cs` — extends `MenuUIBase`; Canvas nested in PlayerRig; subscribes to `BuildingSystem.OnBuildingMenuToggleRequested`; has `_itemRegistry` field; tracks `_rows` list; subscribes to `InventorySystem.OnInventoryReady/Destroyed` + slot changes to call `RefreshAllRows()` live; backdrop + cursor logic inherited from `MenuUIBase`
  - `Assets/Scripts/UI/Building/BuildingMenuPieceRowUI.cs` — icon-based row (mirrors CraftingRecipeRowUI); fields: `_icon` (building icon), `_nameText` (building name), `_ingredientContainer` (HLG), `_ingredientIconPrefab` (IngredientIconPrefab), `_selectButton`; `Init(def, itemRegistry)` spawns ingredient icons + calls `Refresh()`; `Refresh()` reads `InventorySystem.LocalPlayer.CountItem` per ingredient → green/red via `IngredientIconUI`; reuses `IngredientIconUI` + `IngredientIconPrefab.prefab` from crafting system; Select → `BuildingSystem.EnterPlacementMode`
...
- Prefabs: `Assets/App/Prefabs/Buildings/StickFence.prefab` (5× branch, X rot 90°, 0.18 m spacing), `Assets/App/Prefabs/Buildings/StoneWall.prefab` (4× rock, 2×2 grid 0.35×0.25 m); `BuildingMenuUI.prefab` follows **Standard Panel Hierarchy** (see above); `BuildingPieceRowPrefab.prefab` — HLG layout: `Icon` (60×60 Image) | `Name` (TMP_Text) | `IngredientContainer` (HLG, flexibleWidth) | `SelectButton`; wire `BuildingMenuPieceRowUI` fields in Inspector; `_ingredientIconPrefab` → `Assets/App/Prefabs/UI/IngredientIconPrefab.prefab` (shared with crafting)
